...
import { readScrapedJobs, runScraper } from '../lib/scraper';
...
const LOG_PATH = path.join(__dirname, 'apply_actions.log');
...
function formatJobsForPrompt(jobs: any[], limit = 40): string {
  if (!jobs || jobs.length === 0) return 'No scraped jobs available.';
  return jobs
    .slice(0, limit)
    .map((job, i) => {
      const parts = [
      ...
      if (job.tags) parts.push(`   Tags: ${job.tags.slice(0, 6).join(', ')}`);
...
router.post('/apply-preview', async (req, res, next) => {
  try {
    ...
    if (cover_letter) promptParts.push(`Cover letter:\n${cover_letter}`);
    if (resume_bullet) promptParts.push(`Resume bullet:\n${resume_bullet}`);
...
router.post('/find-jobs', async (req, res, next) => {
  try {
    ...
    const scraperResult = readScrapedJobs();
