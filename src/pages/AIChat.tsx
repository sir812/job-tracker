    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-2">
        ...
        <div className="flex flex-col gap-1 md:flex-row md:items-end md:justify-between">
          <div>
            <h2 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">Smarter AI Chatbot</h2>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
              Use job context, resume bullets, and targeted modes for stronger prompts and cleaner outputs.
            </p>
          </div>
          <Link to="/jobs" className="text-xs font-semibold text-cyan-600 hover:text-cyan-500 dark:text-cyan-400 dark:hover:text-cyan-300 transition-colors">
            Pull from your jobs
          </Link>
        </div>
        ...
        <Modal isOpen={applyModalOpen} onClose={() => setApplyModalOpen(false)} title="Application preview" size="lg">
          <div className="space-y-4">
            <div className="text-sm text-slate-900 dark:text-white">
              ...
              <pre className="whitespace-pre-wrap text-xs bg-slate-50 border border-slate-200 text-slate-800 dark:bg-black/70 dark:border-neutral-800 dark:text-slate-100 mt-2">{applyPreviewText}</pre>
            </div>

            {generatedCoverLetter ? (
              <div className="text-sm text-slate-900 dark:text-white">
                <p className="font-semibold text-slate-900 dark:text-white">Generated Cover Letter</p>
                <pre className="whitespace-pre-wrap text-xs bg-slate-50 border border-slate-200 text-slate-800 dark:bg-black/70 dark:border-neutral-800 dark:text-slate-100 mt-2">{generatedCoverLetter}</pre>
              </div>
            ) : null}
      ...
      <div className="grid gap-6 lg:grid-cols-[1.55fr_1fr] items-start">
        <Card className="p-0 overflow-hidden flex flex-col min-h-[74vh] bg-white border-slate-200 dark:bg-gradient-to-br dark:from-black dark:via-black dark:to-neutral-950 dark:border-neutral-800">
          <div className="flex items-center justify-between gap-4 px-5 py-4 border-b border-slate-200 bg-slate-50/80 dark:border-neutral-800 dark:bg-black/70 backdrop-blur-sm">
            <div className="flex items-center gap-3 min-w-0">
              <div className="w-10 h-10 rounded-2xl bg-slate-100 border border-cyan-200 flex items-center justify-center text-cyan-600 shrink-0 dark:bg-black dark:border-cyan-500/20 dark:text-cyan-400">
                <Bot className="w-5 h-5" />
              </div>
              <div className="min-w-0">
                <h3 className="text-sm font-bold text-slate-900 dark:text-white truncate">Job Tracker AI</h3>
                <p className="text-[11px] text-slate-500 dark:text-slate-400 truncate">Connected to backend AI routes with smart job context</p>
              </div>
            </div>
                ...
                return (
                  <button
                    ...
                    className={`flex items-start gap-3 p-3 rounded-2xl border transition-all text-left ${
                      active
                        ? "bg-cyan-50/60 border-cyan-200 text-cyan-950 shadow-none dark:bg-cyan-500/10 dark:border-cyan-400/30 dark:text-slate-100 dark:shadow-lg dark:shadow-black/20"
                        : "bg-slate-50 border-slate-200 hover:bg-slate-100 text-slate-700 dark:bg-black/70 dark:border-neutral-800 dark:hover:bg-black dark:hover:border-neutral-700 dark:text-slate-400"
                    }`}
                  >
                    <div className={`mt-0.5 shrink-0 ${active ? "text-cyan-600 dark:text-cyan-300" : "text-slate-400 dark:text-slate-500"}`}>{item.icon}</div>
                    <div className="min-w-0">
                      <p className="text-xs font-bold uppercase tracking-wider">{item.label}</p>
                      <p className="text-[11px] mt-1 text-slate-500 dark:text-slate-400 leading-relaxed">{item.description}</p>
                    </div>
                  </button>
                );
              })}
          ...
          <div className="flex-1 overflow-y-auto px-4 md:px-5 py-5 space-y-4">
              ...
              <div key={message.id} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}>
                <div
                  className={`max-w-[88%] rounded-3xl px-4 py-3 border shadow-lg backdrop-blur-sm ${
                    message.role === "user"
                        ? "bg-cyan-50/60 border-cyan-200 text-slate-800 dark:bg-cyan-500/15 dark:border-cyan-400/20 dark:text-slate-100"
                        : "bg-slate-100 border-slate-200 text-slate-800 dark:bg-black/70 dark:border-neutral-800 dark:text-slate-100"
                  }`}
                >
                  <div className="flex items-center gap-2 mb-2 text-[10px] uppercase tracking-[0.2em] font-bold">
                    {message.role === "user" ? (
                      <>
                        <User2 className="w-3.5 h-3.5 text-cyan-600 dark:text-cyan-400" />
                        <span className="text-cyan-700 dark:text-cyan-300">You</span>
                      </>
                    ) : (
                      <>
                        <Bot className="w-3.5 h-3.5 text-cyan-600 dark:text-cyan-400" />
                        <span className="text-cyan-700 dark:text-cyan-300">Assistant</span>
                        {message.tag ? <span className="text-slate-500 dark:text-slate-400">/ {message.tag}</span> : null}
                      </>
                    )}
                  </div>
                  <p className="text-sm leading-relaxed whitespace-pre-wrap text-slate-800 dark:text-slate-100">{message.content}</p>
                  <span className="mt-2 block text-[10px] font-medium text-slate-400 dark:text-slate-500">{formatTime(message.timestamp)}</span>
                </div>
              </div>
            ))}
            ...
            {loading && (
              <div className="flex justify-start">
                <div className="rounded-3xl px-4 py-3 bg-slate-100 border border-slate-200 text-slate-800 dark:bg-black/70 dark:border-neutral-800 dark:text-slate-100 max-w-[88%]">
                  ...
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="text-slate-600 dark:text-slate-400 ml-2">Thinking...</span>
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="border-t border-slate-200 bg-slate-50/80 dark:border-neutral-800 dark:bg-black/80 backdrop-blur-sm p-4 md:p-5 space-y-4">
        ...
        <div className="flex flex-col gap-6 sticky top-6">
          <Card className="flex flex-col gap-4">
            <div className="flex items-center gap-2 border-b border-slate-200 dark:border-neutral-800 pb-3">
              ...
              <h3 className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wider">Recent Jobs</h3>
            </div>
            <div className="space-y-3 max-h-[320px] overflow-y-auto pr-1">
              {recentJobs.length === 0 ? (
                <p className="text-xs text-slate-500 dark:text-slate-400 py-4 text-center">No jobs available</p>
              ) : (
                recentJobs.map((job) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left p-3 rounded-2xl border transition-all ${
                        active
                          ? "bg-cyan-50 border-cyan-200 dark:bg-cyan-500/10 dark:border-cyan-400/30"
                          : "bg-slate-50 border-slate-200 hover:bg-slate-100 dark:bg-black/70 dark:border-neutral-800 dark:hover:bg-black dark:hover:border-neutral-700"
                      }`}
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          <h4 className="text-sm font-semibold text-slate-900 dark:text-white truncate">{job.company}</h4>
                          <p className="text-xs text-slate-600 dark:text-slate-300 mt-1 truncate">{job.role}</p>
                          <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-2 truncate">{job.location}</p>
                        </div>
                        <span className="shrink-0 text-[10px] font-bold uppercase tracking-wider text-cyan-700 bg-cyan-50 border border-cyan-200 px-2 py-1 rounded-full dark:text-cyan-300 dark:bg-black dark:border-cyan-500/20">
                          {job.status}
                        </span>
                      </div>
                    </button>
          ...
          <Card className="flex flex-col gap-4">
            <div className="flex items-center gap-2 border-b border-slate-200 dark:border-neutral-800 pb-3">
              ...
              <h3 className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wider">How It Works</h3>
            </div>
            <div className="space-y-3 text-sm text-slate-500 dark:text-slate-400">
              <div className="p-3 rounded-xl bg-slate-50 border border-slate-200 dark:bg-black/70 dark:border-neutral-800">
                <p className="font-semibold text-slate-900 dark:text-white">1. Pick a mode</p>
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">General chat, cover letter, bullet rewrite, job fit, or interview prep.</p>
              </div>
              <div className="p-3 rounded-xl bg-slate-50 border border-slate-200 dark:bg-black/70 dark:border-neutral-800">
                <p className="font-semibold text-slate-900 dark:text-white">2. Add job context</p>
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Select a recent job to auto-fill company, role, status, notes, and tags.</p>
              </div>
              <div className="p-3 rounded-xl bg-slate-50 border border-slate-200 dark:bg-black/70 dark:border-neutral-800">
                <p className="font-semibold text-slate-900 dark:text-white">3. Generate smarter output</p>
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">The frontend calls a specialized endpoint based on your mode.</p>
              </div>
            </div>
          </Card>
        </div>
